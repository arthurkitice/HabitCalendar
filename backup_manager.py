...
def validate_backup(path: str) -> bool:
    try:
        # Usamos o closing para garantir que o arquivo do backup será 
        # fechado e liberado da memória, independente de dar erro ou não.
        with closing(sqlite3.connect(path)) as conn:
            cursor = conn.cursor()
            
            # Consulta a tabela mestre do SQLite para pegar os nomes de todas as tabelas
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
            
            # fetchall() retorna uma lista de tuplas: [('trackers',), ('years',), ...]
            # Usamos Set Comprehension para extrair só o texto e já virar um conjunto (set)
